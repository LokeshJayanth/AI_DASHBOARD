...
def clean_dataframe(df):
    """
    🔥 UNIVERSAL DATA CLEANING ENGINE - Statistical Approach
    Works for ANY dataset using general statistical principles
    
    No dataset-specific rules - uses:
    - IQR method for outlier detection
    - Automatic numeric/categorical detection
    - Median for numeric missing values
    - Mode for categorical missing values
    - Automatic date conversion
    
    Perfect for: Customer, Sales, Student, Hospital, ANY dataset
    """
    ...
    # ============================================
    # 🔹 STEP 2: Replace ALL Missing Value Representations
    # ============================================
    null_values = ["", " ", "  ", "NULL", "null", "Null", "NaN", "nan", "N/A", "n/a", "#N/A", "NA", "None", "none", "-", "--", "?", "Not Available", "not available"]
    df_clean.replace(null_values, np.nan, inplace=True)
    
    # Convert string 'nan' variations to actual NaN
    ...
    # ============================================
    # 🔹 STEP 3: Remove Completely Empty Rows/Columns
    # ============================================
    ...
    for col in df_clean.select_dtypes(include=['object']).columns:
        ...
        # Replace 'nan' string back to NaN after stripping
        df_clean[col] = df_clean[col].replace('nan', np.nan)
    
    # ============================================
    # 🔹 STEP 6: Automatic Type Detection & Conversion
    # ============================================
    for col in df_clean.columns:
        # Try to convert to numeric (use coerce to turn invalid to NaN)
        numeric_col = pd.to_numeric(df_clean[col], errors='coerce')
        
        # If more than 50% values could be converted, it's numeric
        if numeric_col.notna().sum() > len(df_clean) * 0.5:
            df_clean[col] = numeric_col
    
    # ============================================
    # 🔹 STEP 7: Auto-Detect and Convert Date Columns
    # ============================================
    for col in df_clean.columns:
        # Check if column might be a date
        if df_clean[col].dtype == 'object' and any(keyword in col.lower() for keyword in ['date', 'time', 'day', 'dob', 'birth']):
            try:
    ...
    # ============================================
    # 🔹 STEP 8: Fill Missing Values - NUMERIC COLUMNS
    # ============================================
    numeric_cols = df_clean.select_dtypes(include=['int64', 'float64', 'Int64']).columns
    ...
    # ============================================
    # 🔹 STEP 9: Handle Outliers - IQR Method (Universal)
    # ============================================
    for col in numeric_cols:
        if df_clean[col].notna().any() and len(df_clean[col].unique()) > 5:  # Skip if too few values
            # Calculate IQR
            ...
            # Clip outliers to bounds (preserves data, just limits extremes)
            df_clean[col] = np.clip(df_clean[col], lower_bound, upper_bound)
    
    # ============================================
    # 🔹 STEP 10: Fill Missing Values - CATEGORICAL COLUMNS
    # ============================================
    ...
    # ============================================
    # 🔹 STEP 11: Fill Missing Values - DATE COLUMNS
    # ============================================
    date_cols = df_clean.select_dtypes(include=['datetime64']).columns
    
    for col in date_cols:
        if df_clean[col].notna().any() and not df_clean[col].mode().empty:
            # Fill with most common date
            mode_date = df_clean[col].mode()[0]
            df_clean[col].fillna(mode_date, inplace=True)
        else:
            # If all dates are NaN, use today
            df_clean[col].fillna(pd.Timestamp.today(), inplace=True)
        
        # Convert to ISO string format for MySQL compatibility
        df_clean[col] = df_clean[col].dt.strftime('%Y-%m-%d')
    
    # ============================================
    # 🔹 STEP 12: Convert Float to Int Where Appropriate
    # ============================================
...
def get_column_datatypes(df):
    """Detect column datatypes for schema generation"""
    datatypes = {}
    ...
    for col in df.columns:
        ...
        if pd.api.types.is_integer_dtype(df[col]):
            ...
            # Text column - determine VARCHAR size
            max_len = df[col].astype(str).str.len().max() if not df[col].empty else 255
            # Cap at 500, use TEXT for larger
            datatypes[col] = f'VARCHAR({min(int(max_len * 1.5), 500)})' if max_len < 300 else 'TEXT'
